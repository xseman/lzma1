...
export class LZMA {
	...
	#initEncoder(): Encoder {
		return {
			...
			_rangeEncoder: {
				...
				_cache: 0,
				Low: [],
				_cacheSize: function(_cacheSize: any): [number, number] {
					throw new Error("Function not implemented.");
				},
				_position: function(arg0: [number, number], _position: any): unknown {
					throw new Error("Function not implemented.");
				},
			},
	...
	#GetMatches(): number {
		...
		if (matchFinder.HASH_ARRAY) {
			temp = this.#crcTable[matchFinder._bufferBase[cur] & 255] ^ matchFinder._bufferBase[cur + 1] & 255;
			hash2Value = temp & 1023;
			temp ^= (matchFinder._bufferBase[cur + 2] & 255) << 8;
			hash3Value = temp & 65535;
			hashValue = (temp ^ this.#crcTable[matchFinder._bufferBase[cur + 3] & 255] << 5) & matchFinder._hashMask;
		} else {
	...
	#Skip(num: number): void {
		...
		do {
			...
			if (matchFinder.HASH_ARRAY) {
				...
				hash3Value = temp & 65535;
				matchFinder._hash[1024 + hash3Value] = matchFinder._pos;
				hashValue = (temp ^ this.#crcTable[matchFinder._bufferBase[cur + 3] & 255] << 5)
